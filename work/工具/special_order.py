...
def main():
    file_path="C:\\Users\\ly320\\Desktop\\test"
    ...
    data = conn2.select_all("bs_rtime_oper_eff_special_warn")
    data = list(data)
    new_list = ['', '', '订单环节', '警告情况', '超时', '发货仓库', '移出货主', '移出库存地', '指令类型', '指令编号',
                '指令行项|交货序号', '数量', '业务执行状态', '移出物料'
        , '物料描述', '运单编号', '承运商名称', '售达方编号', '售达方名称', '收货人', '电话1', '省份描述', '城市描述',
                '地区描述', '收货地址', '创建日期']
    special_customer_list = []  # 特殊客户列表
    special_customer_list.append(new_list)
    special_area_list = []  # 偏远地区列表
    special_area_list.append(new_list)
    new_file_path = file_path + '.xlsx'
    for i in data:
        if i[2] == '偏远地区':
            special_area_list.append(list(i))
        elif i[2] == '特殊客户':
            special_customer_list.append(list(i))
    write = pd.ExcelWriter(new_file_path)

    data_1 = pd.DataFrame(data)
    data_2 = pd.DataFrame(data=special_area_list)
    data_3 = pd.DataFrame(data=special_customer_list)
    data_1.to_excel(write, sheet_name='sheet1', float_format='%.2f', header=False, index=False)
    data_2.to_excel(write, sheet_name='偏远地区', float_format='%.2f', header=False, index=False)
    data_3.to_excel(write, sheet_name='特殊客户', float_format='%.2f', header=False, index=False)
    write.close()
    name_list = []
    excel_list = ['偏远地区', '特殊客户']
    for j in excel_list:
        df = pd.read_excel(new_file_path, sheet_name=j, usecols='F:Z', keep_default_na=False).astype("str")
        file_path1 = file_path + j + '.xlsx'
